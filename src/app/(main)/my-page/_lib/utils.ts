...
export function getOrderStatusConfig(status?: string) {
  if (!status) return { label: "알 수 없음", colorClass: "border-gray-600/30 bg-gray-600/20 text-gray-400" };
  return ORDER_STATUS_MAP[status] ?? { label: status, colorClass: "border-gray-600/30 bg-gray-600/20 text-gray-400" };
}
